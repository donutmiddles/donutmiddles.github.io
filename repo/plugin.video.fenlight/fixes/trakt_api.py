...
def get_trakt_all_pages(params, limit=250):
	...
	# This also loops every page and preserves sort headers when callers user method='sort_by_headers'.
	page_no = 1
	all_results = []
	extra_params = dict(params.get('params', {}))
	extra_params['limit'] = min(int(extra_params.get('limit', limit)), 250)
	while True:
		...
		if isinstance(result, dict) and 'data' in result:
			...
			sort_how = result.get('sort_how', sort_how)
			page_results = resut.get('data') or []
		else:
			page_results = result
...
def trakt_lists_with_media(media_type, imdb_id):
	...
	params = {'path': '%s/%s/lists/personal', 'path_insert': (media_type, imdb_id), 'params': {'limit': 250, 'pagination': False}}
