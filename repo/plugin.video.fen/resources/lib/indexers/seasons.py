...
def build_season_list(params):
	def _process():
		...
		for item in season_data:
			try:
				...
				cm_extend([
				['extras', ('[B]Extras[/B]', 'RunPlugin(%s)' % build_url({'mode': 'extras_menu_choice', 'tmdb_id': tmdb_id, 'media_type': 'tvshow'}))],
				['cast', ('[B]Cast[/B]', 'RunPlugin(%s)' % build_url({'mode': 'tmdb_cast_dialog_image_results', 'media_type': 'tvshow', 'tmdb_id': tmdb_id}))],
				['more_info', ('[B]More Info[/B]', 'RunPlugin(%s)' % build_url({'mode': 'media_extra_info_choice', 'media_type': 'tvshow', 'tmdb_id': tmdb_id}))],
				['options', ('[B]Options[/B]', 'RunPlugin(%s)' % build_url({'mode': 'options_menu_choice', 'content': 'season', 'tmdb_id': tmdb_id, 'poster': show_poster}))]
						])
				...
				info_tag.setPlaycount(playcount), info_tag.setGenres(genre)
	...
	orig_title, show_plot = meta_get('original_title', ''), meta_get('plot')
	str_tmdb_id, str_tvdb_id, genre = str(tmdb_id), str(tvdb_id), meta_get('genre')
