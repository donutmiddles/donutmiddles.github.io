...
def extras_images_choice(params):
	...
	choices = [{'line1': choice_names[i], 'icon': kodi_utils.get_icon(i), 'type': i} for i in params['choices']]
...
def browse_more_choice(params):
	...
	if media_type == 'single_episode': choices = [('Browse Seasons', 'seasons'), ('Browse Episodes', 'episodes')]
		...
		if settings.ai_model_active(): choices.append(('Similar', 'similar'))
...
def playback_filters_choice(params):
	...
	choices = [dict(i, **{'status': settings.filter_status(i['value'])}) for i in choices]
...
def sync_databases_choice(params):
	...
	current_settings = settings.database_sync_included()
...
def personallists_manager_choice(params):
	...
	if action == 'add_new':
		...
		try: list_name = kodi_utils.select_dialog([i[1] for i in choices], **kwargs)
		except: return
...
def preferred_filters_choice(params):
    ...
    def _param_choices(choice):
        ...
        param_kwargs = {'items': json.dumps(param_list_items), 'multi_line': 'true', 'heading': 'Choose Sort to Top Params', 'narrow_window': 'true'}
    ...
    kwargs = {'items': json.dumps(list_items), 'multi_line': 'true', 'heading': 'Choose Sort to Top Params', 'narrow_window': 'true'}
    choice = kodi_utils.select_dialog(choices, **kwargs)
...
def extras_lists_choice(params={}):
	...
	choice = kodi_utils.select_dialog(all_items, **kwargs)
	if choice  == []:
		kodi_utils.ok_dialog(text='You must select at least 1 item')
		...
		for _type in ('movie', 'tvshow'):
			...
			for item in range(10, 18):
				...
				try:
					...
					set_setting(setting_id.replace('fen.', ''), default_setting_values(setting_id)['setting_default'])
					button_action = get_setting('fen.%s' % setting_id)
					button_label = extras_button_label_values[_type][button_action]
...
def extras_ratings_choice(params={}):
	...
	list_items = [{'line1': i[0], 'icon': 'fen_flags/ratings/%s' % i[2]} for i in choices]
...
def playback_choice(params):
	...
	if debrid_for_ext_cache_check():
		items.append({'line': 'Rescrape with External Cache Check [B]OFF[/B]', 'function': 'rescrape_external_cache_check_off'})
...
def set_quality_choice(params):
	...
	fl = ['4K', '1080p', '720p', 'SD']
	q_setting = get_setting('fen.%s' % quality_setting).split(', ')
...
def enable_scrapers_choice(params):
	icon = params.get('icon', None) or kodi_utils.get_icon('fen')
	scrapers = ['external', 'easynews', 'rd_cloud', 'pm_cloud', 'oc_cloud', 'tb_cloud', 'folders']
	cloud_scrapers = {'rd_cloud': 'rd.enabled', 'pm_cloud': 'pm.enabled', 'oc_cloud': 'oc.enabled', 'tb_cloud': 'tb.enabled'}
	scraper_names = ['EXTERNAL SCRAPERS', 'EASYNEWS', 'RD CLOUD', 'PM CLOUD', 'OC CLOUD', 'TB CLOUD', 'FOLDERS 1-5']
...
def options_menu_choice(params, meta=None):
	...
	from modules.utils import get_datetime
	from modules import metadata
	# from modules.meta_lists import mood_matrix
	# listing = []
	# listing_append = listing.append
	# all_moods = mood_matrix()
	# for category in all_moods:
	# 	sub_categories = all_moods[category]
	# 	for sub_category, sub_info in sub_categories.items():
	# 		sub_description = sub_info['description']
	# 		sub_keywords = sub_info['keywords']
	# 		listing.append({'display': '%s: %s' % (category, sub_category), 'sub_info': sub_info, 'category': category, 'sub_category': sub_category})
	
	# list_items = [{'line1': item['display'], 'line2': item['sub_info']['description'], 'icon': None} for item in listing]
	# heading = 'Choose Your Current Mood...'
	# kwargs = {'items': json.dumps(list_items), 'heading': heading, 'multi_line': 'true'}
	# choice = kodi_utils.select_dialog([i for i in listing], **kwargs)
	# logger('choice', choice)
	# logger('keywords', choice['sub_info']['keywords'])
	# return
	...
	season, episode, single_ep_list = params_get('season', ''), params_get('episode', ''), ('progress', 'next')
