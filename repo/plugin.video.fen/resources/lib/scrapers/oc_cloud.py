...
class source:
				...
				for item in self.scrape_results:
					try:
						...
						source_item = {'name': file_name, 'display_name': display_name, 'quality': video_quality, 'size': size, 'size_label': '%.2f GB' % size,
									...
									'scrape_provider': self.scrape_provider, 'folder_id': folder_id, 'cache_type': cache_type, 'direct_debrid_link': True}
						yield source_item
					except: pass
	...
	def _scrape_cloud(self):
		try:
			try:
				my_cloud_files = Offcloud.user_cloud()
			...
			for item in my_cloud_files:
				...
				if not folder_name: results_append(item['requestId'])
	...
	def _scrape_folders(self, folder_info):
		try:
			folder_files = Offcloud.user_cloud_info(folder_info)['files']
			contents = [i for i in folder_files if i['path'].lower().endswith(tuple(self.extensions))]
			scrape_results_append = self.scrape_results.append
			for item in contents:
				normalized = self._get_filename(item['path'])
				if self.media_type == 'episode' and not source_utils.seas_ep_filter(self.season, self.episode, normalized): continue
				if item['path'].lower() not in [d['path'].lower() for d in self.scrape_results]:
	...
	# 	try:
	# 		my_downloads = Offcloud.downloads()
