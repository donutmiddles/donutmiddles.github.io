...
class Discover(BaseDialog):
	...
	def years(self):
		if self.check_filter_clashes('with_year_end', 'with_released', 'Released Only'): return
	...
	def certifications(self):
		...
		choice = self.selection_dialog(self.chosen_item['label'], [{'name': i['name']} for i in certifications], certifications)
		if choice != None: self.set_key_values(self.chosen_item['url_insert'] % str(choice['id']), str(choice['name']))

	def certification_and_lowers(self):
		certifications = meta_lists.movie_certifications()
		movie_and_lower_certifications = [{'name': '%s and lower' % i['name'], 'id': i['id']} for i in certifications if not i['id'] == 'g']
	...
	def released(self):
		if self.check_filter_clashes('with_released', 'with_year_end', 'Year End'): return
	...
	def check_filter_clashes(self, using_filter, check_filter, check_filter_name):
		if self.chosen_item['key'] == using_filter and check_filter in self.get_active_attributes():
			ok_dialog(text='Remove [B]%s[/B] filter to set this filter' % check_filter_name.upper())
			return True
		return False
