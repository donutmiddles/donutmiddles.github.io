...
from apis.imdb_api import imdb_people, imdb_video_info
...
from modules.settings import tmdb_api_key, easynews_authorized, rpdb_info
from modules.kodi_utils import addon_fanart, show_busy_dialog, hide_busy_dialog, get_icon
# from modules.kodi_utils import logger

class People(BaseDialog):
	more_from_movies_id, more_from_tvshows_id, trivia_id, quotes_id, imdb_videos_id, more_from_director_id = 2050, 2051, 2052, 2053, 2054, 2055
	...
	def __init__(self, *args, **kwargs):
		...
		self.tasks = (self.set_infoline1, self.make_imdb_extras, self.make_movies, self.make_tvshows, self.make_director)
	...
	def onClick(self, controlID):
		self.control_id = None
		if controlID in (10, 11, 50):
			if controlID == 10:
				type_choices = ['tmdb', 'imdb']
				if easynews_authorized(): type_choices.append('easynews')
				image_type = dialogs.extras_images_choice({'choices': type_choices})
				if image_type == 'tmdb':
				...
				elif image_type == 'imdb':
					self.Images({'mode': 'imdb_image_results', 'imdb_id': self.person_imdb_id, 'rootname': self.person_name})
				elif image_type == 'easynews':
			...
			else: self.show_text_media(text=self.person_biography)
	...
	def onAction(self, action):
		...
		if action == self.info_action:
			...
			if not focus_id in (self.more_from_movies_id, self.more_from_tvshows_id, self.more_from_director_id): return
			show_busy_dialog()
			from modules.metadata import movie_meta, tvshow_meta
			chosen_listitem = self.get_listitem(focus_id)
			media_type = 'movie' if focus_id in (self.more_from_movies_id, self.more_from_director_id) else 'tvshow'
		...
		if action in self.selection_actions:
			...
			if self.control_id in (self.more_from_movies_id, self.more_from_tvshows_id, self.more_from_director_id):
				if self.control_id in (self.more_from_movies_id, self.more_from_director_id): media_type = 'movie'
				else: media_type = 'tvshow'
				self.set_current_params()
				self.new_params = {'mode': 'extras_menu_choice', 'tmdb_id': int(chosen_var), 'media_type': media_type, 'stacked': 'true'}
				return window_manager(self)
			elif self.control_id in (self.trivia_id, self.quotes_id):
				return self.select_item(self.control_id, self.show_text_media(text=self.get_attribute(self, chosen_var), current_index=self.get_position(self.control_id)))
			elif self.control_id == self.imdb_videos_id:
				try:
					video_info = imdb_video_info(chosen_var)[0]
	...
	def make_imdb_extras(self):
		imdb_extras = imdb_people(self.imdb_id)
		for item, data in imdb_extras.items():
			if item == 'videos': continue
	...
	def make_imdb_videos(self, all_videos):
		def builder():
			for item in all_videos:
				try:
					listitem = self.make_listitem()
					listitem.setProperties({'name': item['name'], 'thumbnail': item['thumb'], 'key_id': item['id']})
					yield listitem
				except: pass
		try:
	...
	def make_common_text(self, text_type, text_data):
		...
		text_title_lookup = 'imdb_%s.number'
		def builder():
	...
	def make_person_data(self):
		...
		if not self.person_id:
			self.notification('No Results')
			return self.close()
		person_info = tmdb_people_full_info(self.person_id)
		self.person_name = unquote(person_info['name'])
		self.imdb_id = person_info['imdb_id']
		self.person_imdb_id = person_info['imdb_id'] or imdb_people_id(self.person_name)
	...
	def make_more_from(self, media_type):
		...
		try:
			if media_type == 'movie':
				list_type, _id, data, date_key = media_type, self.more_from_movies_id, self.movie_data, 'release_date'
				try: data = [i for i in data if not 99 in i['genre_ids'] and not i['character'].lower() in roles_exclude]
				except: pass
			elif media_type == 'tvshow':
				list_type, _id, data, date_key = media_type, self.more_from_tvshows_id, self.tvshow_data, 'first_air_date'
			...
			else: list_type, _id, data, date_key = 'movie', self.more_from_director_id, self.director_data, 'release_date'
	...
	def set_current_params(self, set_starting_position=True):
		self.current_params = {'mode': 'person_menu_choice', 'key_id': self.key_id, 'actor_name': self.person_name, 'actor_image': self.person_image, 'stacked': 'true',
								'actor_id': self.person_id, 'reference_tmdb_id': self.reference_tmdb_id}
