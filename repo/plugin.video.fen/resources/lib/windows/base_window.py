...
def window_manager(obj):
	def play_video(params):
		...
		if 'plugin.video.youtube' in url:
			if not kodi_utils.addon_installed('plugin.video.youtube') or not kodi_utils.addon_enabled('plugin.video.youtube'):
				kodi_utils.notification('Youtube Plugin needed for playback')
				return runner(remove_from_stack())
		...
		runner(remove_from_stack())

	def close():
		obj.close()
		kodi_utils.clear_property('fen.window_loaded')
		kodi_utils.clear_property('fen.window_stack')

	def monitor():
		timer = 0
		while not kodi_utils.get_property('fen.window_loaded') == 'true' and timer <= 5:
			kodi_utils.sleep(50)
			timer += 0.05
		kodi_utils.hide_busy_dialog()
		obj.close()
	...
	def runner(params):
		try:
			...
			if mode == 'extras_menu_choice':
				...
				from indexers.dialogs import person_menu_choice
				person_menu_choice(params)
	...
	def get_stack():
		try: window_stack = json.loads(kodi_utils.get_property('fen.window_stack'))
		except: window_stack = []
		return window_stack

	def add_to_stack(params):
		window_stack.append(params)
		kodi_utils.set_property('fen.window_stack', json.dumps(window_stack))
...
class BaseDialog(xbmcgui.WindowXMLDialog):
	...
	def set_home_property(self, prop, value):
		kodi_utils.set_property('fen.%s' % prop, value)

	def get_home_property(self, prop):
		return kodi_utils.get_property('fen.%s' % prop)

	def clear_home_property(self, prop):
		return kodi_utils.clear_property('fen.%s' % prop)
...
class FontUtils:
	def execute_custom_fonts(self, skin_files=[]):
		...
		if not skin_files:
			kodi_utils.set_property('fen.current_skin', self.current_skin)
			kodi_utils.set_property('fen.current_font', self.current_font)
		skin_files = skin_files or kodi_utils.list_dirs(kodi_utils.translate_path('special://home/addons/plugin.video.fen/resources/skins/Default/1080i/'))[1]
	...
	def skin_change_check(self):
		...
		if self.current_skin != kodi_utils.get_property('fen.current_skin') or self.current_font != kodi_utils.get_property('fen.current_font'): return True
		return False
	...
	def replace_font(self, window, replacement_values):
		file = kodi_utils.translate_path('special://home/addons/plugin.video.fen/resources/skins/Default/1080i/' + window)
...
class ExtrasUtils:
	def __init__(self):
		...
		2055: {'insert_values': {'heading_label': self.media_heading.format(label='Similar', label_lookup='ai_similar'),
								...
								'template': self.thumb_media_template()},
		2060: {'insert_values': {'heading_label': self.media_heading.format(label='Reviews', label_lookup='imdb_reviews'), 'align': 'center'},
								'template': self.text_media_template()},
		2061: {'insert_values': {'heading_label': self.media_heading.format(label='Comments', label_lookup='trakt_comments'), 'align': 'center'},
								'template': self.text_media_template()},
		2062: {'insert_values': {'heading_label': self.media_heading.format(label='Trivia', label_lookup='imdb_trivia'), 'align': 'center'},
								'template': self.text_media_template()},
		2063: {'insert_values': {'heading_label': self.media_heading.format(label='Blunders', label_lookup='imdb_blunders'), 'align': 'center'},
								'template': self.text_media_template()},
		2064: {'insert_values': {'heading_label': self.media_heading.format(label='FAQs', label_lookup='imdb_faqs'), 'align': 'center'},
								'template': self.text_media_template()},
		2065: {'insert_values': {'heading_label': self.media_heading.format(label='Quotes', label_lookup='imdb_quotes'), 'align': 'center'},
								'template': self.text_media_template()},
		2066: {'insert_values': {'heading_label': self.media_heading.format(label='Parental Guide', label_lookup='imdb_parentsguide')},
								'template': self.parentsguide_template()},
		2080: {'insert_values': {'heading_label': self.media_heading.format(label='IMDb Videos', label_lookup='imdb_videos'),
								'highlight_label': self.wide_thumb_highlight.format(label='IMDb Videos')},
								'template': self.wide_thumb()},
		2081: {'insert_values': {'heading_label': self.media_heading.format(label='Youtube Videos', label_lookup='youtube_videos'),
								'highlight_label': self.wide_thumb_highlight.format(label='Youtube Videos')},
								'template': self.wide_thumb()},
		2082: {'insert_values': {'heading_label': self.media_heading.format(label='More from Year', label_lookup='more_from_year'),
								'highlight_label': self.media_highlight.format(label='More from Year')},
								'template': self.thumb_media_template()},
		2083: {'insert_values': {'heading_label': self.media_heading.format(label='More from Genres', label_lookup='more_from_genres'),
								'highlight_label': self.media_highlight.format(label='More from Genres')},
								'template': self.thumb_media_template()},
		2084: {'insert_values': {}, 'template': self.collection_template()}
						}

	def run(self):
		finished_templates = []
		skin_file = 'extras.xml'
		file = kodi_utils.translate_path('special://home/addons/plugin.video.fen/resources/skins/Default/1080i/%s' % skin_file)
				...
				<control type="group">
				    ...
				    <control type="group">
				        ...
				        <control type="fixedlist" id="{container_no}">
				            ...
				            <itemlayout height="360" width="236">
				                <control type="image">
				                    ...
				                    <texture diffuse="fen_diffuse/poster-50.png" background="true">$INFO[ListItem.Property(thumbnail)]</texture>
				                </control>
				            </itemlayout>
				            <focusedlayout height="360" width="236">
				                <control type="image">
				                    ...
				                    <height>350</height>
				                    <width>236</width>
				                    <texture colordiffuse="FFCCCCCC">fen_diffuse/poster-50.png</texture>
				        ...
				        <control type="image">
				            ...
				            <width>25</width>
				            <height>25</height>
				            <texture colordiffuse="CCCCCCCC" background="true">fen_common/arrow_left.png</texture>
				        ...
				        <control type="image">
				            ...
				            <width>25</width>
				            <height>25</height>
				            <texture colordiffuse="CCCCCCCC" background="true" flipx="true">fen_common/arrow_left.png</texture>
				        ...
				        <control type="panel" id="{container_no}">
				            ...
				            <itemlayout height="380" width="590">
				                <control type="image">
				                    <height>360</height>
				                    <width>580</width>
				                    <texture colordiffuse="$INFO[Window(10000).Property(fen.window_theme.extras)]" border="30">fen_common/circle.png</texture>
				                </control>
                ...
                <control type="group">
                    ...
                    <control type="group">
                        ...
                        <control type="button" id="{container_no}">
                            ...
                            <texturenofocus colordiffuse="$INFO[Window(10000).Property(fen.window_theme.extras)]" border="30">fen_common/circle.png</texturenofocus>
                        </control>
                        <control type="textbox">
                            <top>65</top>
                            <left>15</left>
        ...
        <control type="group">
            ...
            <control type="group">
                ...
                <control type="image">
                    ...
                    <texture>$INFO[Window(10000).Property(fen.addon_icon)]</texture>
                </control>
                <control type="group">
                ...
                <control type="group">
                    ...
                    <control type="image">
                        ...
                        <height>35</height>
                        <aspectratio>keep</aspectratio>
                        <texture>fen_common/info.png</texture>
                        <visible>!String.IsEqual(ListItem.Property(info_alert),)</visible>
                    </control>
                    <control type="label">
            ...
            <control type="group">
                <control type="image" id="200">
                    ...
                    <height>1000</height>
                    <aspectratio>keep</aspectratio>
                    <texture diffuse="fen_diffuse/poster-50.png" background="true" />
                </control>
                <control type="group">
                    ...
                    <control type="group">
                        ...
                        <control type="group">
                            ...
                            <control type="image">
                                ...
                                <height>348</height>
                                <aspectratio>keep</aspectratio>
                                <texture diffuse="fen_diffuse/poster-50.png" background="true">$INFO[Window.Property(more_from_collection.poster)]</texture>
                            </control>
                            <control type="textbox">
                                <left>235</left>
                                <top>6</top>
                        ...
                        <control type="fixedlist" id="{container_no}">
                            ...
                            <itemlayout height="360" width="392">
                                <control type="image">
                                    ...
                                    <aspectratio>keep</aspectratio>
                                    <texture diffuse="fen_diffuse/landscape.png" background="true">$INFO[ListItem.Property(thumbnail)]</texture>
                                </control>
                            </itemlayout>
                            <focusedlayout height="360" width="392">
                                <control type="image">
                                    ...
                                    <height>296</height>
                                    <width>393</width>
                                    <texture colordiffuse="FFCCCCCC">fen_diffuse/landscape.png</texture>
                                    <visible>Control.HasFocus({container_no}) | Control.HasFocus({scrollbar_no})</visible>
                                </control>
                                <control type="image">
                                    <left>8</left>
