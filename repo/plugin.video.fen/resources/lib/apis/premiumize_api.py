...
class PremiumizeAPI:
	def __init__(self):
		self.token = get_setting('fen.pm.token', 'empty_setting')
	...
	def auth(self):
		...
		if self.token:
			...
			set_many([('pm.token', self.token), ('pm.account_id', str(account_info['customer_id'])), ('pm.enabled', 'true')])
			ok_dialog(text='Success')

	def revoke(self):
		set_many([('pm.token', 'empty_setting'), ('pm.account_id', 'empty_setting'), ('pm.enabled', 'false')])
		ok_dialog(text='Premiumize Authorization Reset')
	...
	def headers(self):
		return {'User-Agent': 'Fen for Kodi', 'Authorization': 'Bearer %s' % self.token}
