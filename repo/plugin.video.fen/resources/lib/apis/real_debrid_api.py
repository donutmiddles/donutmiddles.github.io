...
class RealDebridAPI:
	def __init__(self):
		self.client_ID = get_setting('fen.rd.client_id', 'empty_setting')
		if self.client_ID in ('empty_setting', ''): self.client_ID = 'X245A4XAIBGVM'
		url = {'true': 'app.real-debrid.com', 'false': 'api.real-debrid.com'}[get_setting('fen.rd.alternate_base_url', 'false')]
		self.base_url = 'https://%s/rest/1.0/' % url
		self.auth_url = 'https://%s/oauth/v2/' % url
		self.token = get_setting('fen.rd.token', 'empty_setting')
		self.secret = get_setting('fen.rd.secret', 'empty_setting')
		self.refresh = get_setting('fen.rd.refresh', 'empty_setting')
		self.device_code = ''
	...
	def auth(self):
		...
		if self.secret:
			...
			set_many([('rd.client_id', self.client_ID), ('rd.secret', self.secret), ('rd.token', self.token), ('rd.refresh', self.refresh),
				('rd.account_id', username), ('rd.enabled', 'true')])
	...
	def refresh_token(self):
		try:
			...
			self.token = response['access_token']
			self.refresh = response['refresh_token']
			set_many([('rd.token', self.token), ('rd.refresh', self.refresh)])
			return True
		except: return False

	def revoke(self):
		set_many([('rd.client_id', 'empty_setting'), ('rd.secret', 'empty_setting'), ('rd.token', 'empty_setting'), ('rd.refresh', 'empty_setting'),
			('rd.account_id', 'empty_setting'), ('rd.enabled', 'false')])
		ok_dialog(text='Real Debrid Authorization Reset')
