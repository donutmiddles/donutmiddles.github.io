...
def tmdb_movies_discover(query, page_no):
	...
	return lists_cache_object(get_tmdb, string, url, json=True)
	# string = url = 'https://api.themoviedb.org/3/discover/movie?api_key=%s&language=en-US&with_genres=%s&with_keywords=%s&page=%s' % (api_key, '18', '180547|236486', page_no)
	# result = get_tmdb(url)
	# logger('result', result.json())
	# return result.json()
...
def tmdb_movies_premieres(page_no):
	...
	'&with_release_type=1|3|2|4&page=%s' % (api_key, previous_date, current_date, page_no)
	return lists_cache_object(get_data, string, url)
...
def tmdb_movies_genres(genre_id, page_no):
	...
	url = 'https://api.themoviedb.org/3/discover/movie?api_key=%s&with_genres=%s&language=en-US&region=US&with_original_language=en&sort_by=popularity.desc' \
	'&release_date.lte=%s&page=%s' % (api_key, genre_id, get_current_date(), page_no)
	return lists_cache_object(get_data, string, url)
...
def tmdb_movies_search(query, page_no):
	...
	url = 'https://api.themoviedb.org/3/search/movie?api_key=%s&language=en-US&include_adult=true&query=%s&page=%s' % (api_key, query, page_no)
	return lists_cache_object(get_data, string, url)
...
def tmdb_tv_popular(page_no):
	...
	url = 'https://api.themoviedb.org/3/discover/tv?api_key=%s&sort_by=popularity.desc&language=en-US&region=US&with_original_language=en&page=%s' % (api_key, page_no)
	return lists_cache_object(get_data, string, url)
...
def tmdb_tv_popular_today(page_no):
	...
	'&first_air_date.gte=%s&first_air_date.lte=%s&page=%s' % (api_key, previous_date, current_date, page_no)
	return lists_cache_object(get_data, string, url)
...
def tmdb_tv_genres(genre_id, page_no):
	...
	'&include_null_first_air_dates=false&first_air_date.lte=%s&page=%s' % (api_key, genre_id, get_current_date(), page_no)
	return lists_cache_object(get_data, string, url)
...
def tmdb_tv_year(year, page_no):
	...
	'&first_air_date_year=%s&page=%s' % (api_key, year, page_no)
	return lists_cache_object(get_data, string, url)
...
def tmdb_tv_search(query, page_no):
	...
	url = 'https://api.themoviedb.org/3/search/tv?api_key=%s&language=en-US&include_adult=true&query=%s&page=%s' % (api_key, query, page_no)
	return lists_cache_object(get_data, string, url)
...
def tmdb_people_info(query, page_no=1):
	...
	url = 'https://api.themoviedb.org/3/search/person?api_key=%s&language=en&include_adult=true&query=%s&page=%s' % (api_key, query, page_no)
