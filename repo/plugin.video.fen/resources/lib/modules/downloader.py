...
def runner(params):
	...
	if action == 'image':
		...
		try:
			...
			icon = {'Real-Debrid': 'realdebrid', 'Premiumize.me': 'premiumize', 'Torbox': 'torbox', 'Offcloud': 'offcloud'}[provider]
...
def download_threads_manager(multi_downloads, image):
	...
	for item in multi_downloads:
		...
		kodi_utils.set_property('fen.active_queued_downloads', json.dumps(remaining_downloads))
	kodi_utils.clear_property('fen.active_queued_downloads')
...
class Downloader:
	...
	def set_percent_property(self, percent):
		kodi_utils.set_property('fen.%s' % self.final_name, str(percent))
