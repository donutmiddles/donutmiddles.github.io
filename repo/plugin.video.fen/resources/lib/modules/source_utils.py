...
def internal_results(provider, sources):
	set_property('fen.internal_results.%s' % provider, json.dumps(sources))
...
def get_external_cache_status(debrid, unchecked_hashes, data):
	...
	try:
		results = []
		imdb_id = data['imdb']
		debrid_name, services, token = {'Real-Debrid': ('realdebrid', ['torrentio',], get_setting('fen.rd.token'))}[debrid]
