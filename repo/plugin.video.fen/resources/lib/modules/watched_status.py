...
def get_database():
	return connect_database('watched_db')
...
def update_hidden_progress(media_id):
	...
	hide_unhide_progress_items({'action': 'undrop', 'media_type': 'shows', 'media_id': media_id, 'section': 'dropped', 'refresh': 'false'})

@data_sync_wrapper('watched_db')
...
def erase_bookmark_inside(media_type, media_id, season='', episode=''):
	try:
		watched_db = get_database()
		watched_db.execute('DELETE FROM progress where db_type = ? and media_id = ? and season = ? and episode = ?', (media_type, media_id, season, episode))
	except: pass

def batch_erase_bookmark(insert_list, action):
	try:
		watched_db = get_database()
...
def set_bookmark(params):
	try:
		...
		erase_bookmark_inside(media_type, tmdb_id, season, episode)
...
def mark_movie(params):
	...
	watched_status_mark(media_type, tmdb_id, action, title=title)
	refresh_container(refresh)
...
def mark_episode(params):
	...
	watched_status_mark(media_type, tmdb_id, action, season, episode, title)
	update_hidden_progress(tmdb_id)
	refresh_container(refresh)

@data_sync_wrapper('watched_db')
def watched_status_mark(media_type='', media_id='', action='', season='', episode='', title=''):
	try:
		last_played = get_last_played_value()
		dbcon = get_database()
		...
		erase_bookmark_inside(media_type, media_id, season, episode)
		# if media_type == 'episode': clear_cache_watched_tvshow_status()
	except: notification('Error')

@data_sync_wrapper('watched_db')
def batch_watched_status_mark(insert_list, action):
	try:
		dbcon = get_database()
		if action == 'mark_as_watched':
