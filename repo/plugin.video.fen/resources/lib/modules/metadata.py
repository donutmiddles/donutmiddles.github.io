...
def movie_meta(id_type, media_id, api_key, current_date, current_time=None):
	...
	try:
		...
		data_get = data.get
		cast, writer, director, all_trailers, country, country_codes, studio, stinger_keys = [], [], [], [], [], [], [], []
		mpaa, trailer = '', ''
		...
		if release_dates:
			...
			try: mpaa = next(x['certification'] for i in release_results for x in i['release_dates'] if i['iso_3166_1'] == 'US' and x['certification'] != '')
			except: pass
		credits = data_get('credits')
		if credits:
			all_cast = credits.get('cast', None)
			if all_cast:
				try: cast = [{'name': i['name'], 'role': i['character'], 'thumbnail': tmdb_image_url % ('h632', i['profile_path'])if i['profile_path'] else ''} for i in all_cast]
				except: pass
		...
		else: ei_collection_name, ei_collection_id = '', ''
		...
		meta = {'tmdb_id': tmdb_id, 'imdb_id': imdb_id, 'rating': rating, 'tagline': tagline, 'votes': votes, 'premiered': premiered, 'imdbnumber': imdb_id, 'trailer': trailer,
				...
				'clearlogo': clearlogo, 'landscape': landscape, 'keywords': keywords, 'rpdb_poster': rpdb_poster, 'stinger_keys': stinger_keys, 'external_ids': external_ids}
...
def tvshow_meta(id_type, media_id, api_key, current_date, current_time=None):
	...
	try:
		...
		if content_ratings:
			try: mpaa = next((i['rating'] for i in content_ratings['results'] if i['iso_3166_1'] == 'US'), '')
			except: pass
		credits = data_get('credits')
		if credits:
			all_cast = credits.get('cast', None)
		...
		meta = {'tmdb_id': tmdb_id, 'tvdb_id': tvdb_id, 'imdb_id': imdb_id, 'rating': rating, 'plot': plot, 'tagline': tagline, 'votes': votes, 'premiered': premiered, 'year': year,
				...
				'landscape': landscape, 'keywords': keywords, 'rpdb_poster': rpdb_poster, 'external_ids': external_ids}
