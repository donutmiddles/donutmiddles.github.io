...
class MenuEditor:
	def __init__(self, params):
		self.main_list_name_dict = {'RootList': 'Root', 'MovieList': 'Movies', 'TVShowList': 'TV Shows'}
	...
	def restore(self):
		if not kodi_utils.confirm_dialog(): return kodi_utils.notification('Cancelled', 1500)
		self._db_execute('delete', self.active_list, list_type='edited', refresh=False, perform_sync=False)
	...
	def shortcut_folder_rename(self):
		...
		self._db_execute('delete', self.name, list_type='shortcut_folder', refresh=False, perform_sync=False)
	...
	@data_sync_wrapper('navigator_db')
	def _remove_active_shortcut_folder(self, main_menu_items_list, folder_name):
		for x in main_menu_items_list:
			try:
				...
				self._db_execute('set', x[0], new_list, refresh=False, perform_sync=False)
			except: pass
