...
class Sources():
	def __init__(self):
		...
		self.run_recap, self.run_intro, self.run_outro = False, False, False
		...
		self.default_internal_scrapers = ('easynews', 'rd_cloud', 'pm_cloud', 'oc_cloud', 'tb_cloud', 'folders')
		...
		'pm_browse': ('apis.premiumize_api', 'PremiumizeAPI'), 'Offcloud': ('apis.offcloud_api', 'OffcloudAPI'), 'oc_cloud': ('apis.offcloud_api', 'OffcloudAPI'),
	...
	def playback_prep(self, params=None):
		...
		else: self.external_cache_check = True
		if self.play_type:
			if self.play_type == 'autoplay_nextep': self.autoplay_nextep = True
			else: self.autoplay_nextep = False
		else: self.autoplay_nextep = settings.autoplay_next_episode()
		...
		self.filter_size = get_setting('fen.results.filter_size', 'false') == 'true'
		...
		self.include_unknown_size = get_setting('fen.results.size_unknown', 'false') == 'true'
	...
	def sort_results(self, results):
		results = [dict(i, **{'provider_rank': self._get_provider_rank(i['debrid'].lower()), 'quality_rank': self._get_quality_rank(i.get('quality', 'SD')),
							'size_rank': i['size']}) for i in results]
	...
	def filter_results(self, results):
		...
		if self.filter_size:
			...
			max_size = ((0.125 * (0.90 * string_to_float(get_setting('results.line_speed', '20'), '20'))) * duration)/1000
			results = [i for i in results if i['scrape_provider'] == 'folders' or 0.02 <= i['size'] <= max_size]
	...
	def sort_first(self, results):
		try:
			...
			sort_first_scrapers.extend([i for i in self.all_scrapers if i in ('rd_cloud', 'pm_cloud', 'oc_cloud', 'tb_cloud')])
	...
	def internal_sources(self, prescrape=False):
		active_sources = [i for i in self.active_internal_scrapers if i in ['easynews', 'rd_cloud', 'pm_cloud', 'oc_cloud', 'tb_cloud']]
	...
	def get_folderscraper_info(self):
		folder_info = [(get_setting('fen.%s.display_name' % i), i, settings.source_folders_directory(self.media_type, i))
	...
	def display_results(self, results):
		action, chosen_item = open_window(('windows.sources', 'SourcesResults'), 'sources_results.xml',
				results=results, meta=self.meta, episode_group_label=self.episode_group_label, scraper_settings=self.scraper_settings, prescrape=self.prescrape,
				filters_ignored=self.filters_ignored, uncached_results=self.uncached_results, external_cache_check=self.external_cache_check)
	...
	def _process_post_results(self):
		if self.active_external and self.orig_results and self.external_cache_check and debrid.debrid_for_ext_cache_check(self.debrid_enabled) and not self.rescrape_cache_ignored:
			self.rescrape_cache_ignored = True
			self.threads, self.prescrape, self.external_cache_check = [], False, False
			return self.get_sources()
		if self.active_external and not self.orig_results and not self.meta.get('custom_year') and not self.original_year_ignored:
			...
			if imdb_year != self.get_search_year():
				...
				return self.get_sources()
		if self.active_external and not self.rescrape_with_all:
			self.rescrape_with_all = True
			self.threads, self.disabled_ext_ignored, self.prescrape = [], True, False
			return self.get_sources()
		if self.media_type == 'episode' and not self.rescrape_with_episode_group:
	...
	def _process_internal_results(self):
		for i in self.internal_scrapers:
			win_property = kodi_utils.get_property('fen.internal_results.%s' % i)
			if win_property in ('checked', '', None): continue
			try: sources = json.loads(win_property)
			except: continue
			kodi_utils.set_property('fen.internal_results.%s' % i, 'checked')
	...
	def get_meta(self):
		if self.media_type == 'movie': self.meta = metadata.movie_meta('tmdb_id', self.tmdb_id, settings.tmdb_api_key(), get_datetime())
		else:
			try:
				self.meta = metadata.tvshow_meta('tmdb_id', self.tmdb_id, settings.tmdb_api_key(), get_datetime())
	...
	def _clear_properties(self):
		...
		for item in def_internal: kodi_utils.clear_property('fen.internal_results.%s' % item)
		if self.active_folders:
			for item in self.folder_info: kodi_utils.clear_property('fen.internal_results.%s' % item[0])
	...
	def _make_nextep_dialog(self):
		try: action = open_window(('windows.playback_notifications', 'NextEpisode'), 'playback_notifications.xml', meta=self.meta)
	...
	def debridPacks(self, debrid_provider, name, magnet_url, info_hash, download=False):
		...
		debrid_info = {'Real-Debrid': 'rd_browse', 'Premiumize.me': 'pm_browse', 'TorBox': 'tb_browse', 'Offcloud': 'oc_browse'}[debrid_provider]
	...
	def episode_intro_info(self):
		self.intro_checklist = []
		intro_level = settings.skip_episode_intros()
		if intro_level != 0:
			try: info = episode_intros.get_data(self.meta.get('imdb_id'), self.season, self.episode)
			except: info = {}
			recap_info = info.get('recap', '') if intro_level in (2, 3) else {}
			if recap_info: self.start_recap, self.end_recap = recap_info.get('start_sec'), recap_info.get('end_sec')
			else: self.start_recap, self.end_recap = None, None
			intro_info = info.get('intro', '') if intro_level in (1, 3) else {}
			if intro_info: self.start_intro, self.end_intro = intro_info.get('start_sec'), intro_info.get('end_sec')
			else: self.start_intro, self.end_intro = None, None
			outro_info = info.get('outro', '')
			if outro_info: self.start_outro, self.end_outro = outro_info.get('start_sec'), outro_info.get('end_sec')
			else: self.start_outro, self.end_outro = None, None
			self.intro_checklist = [i for i in {3: ['recap', 'intro'], 2: ['recap'], 1: ['intro'], 0: []}[intro_level] if getattr(self, 'start_%s' % i) is not None]
	...
	def play_file(self, results, source={}):
		...
		try:
			kodi_utils.hide_busy_dialog()
			url = None
			if self.media_type == 'episode': Thread(target=self.episode_intro_info).start()
			...
			for count, item in enumerate(items, 1):
				...
				if provider == 'easynews':
					for retry in range(1, 2):
						...
						resolve_item['retry_active'] = True
						processed_items_append(resolve_item)
	...
	def get_resume_status(self, percent):
		...
		# if self.autoplay: return 'resume'
		# return self._make_resume_dialog(percent)
	...
	def resolve_sources(self, item, meta=None):
		...
		try:
			if 'cache_provider' in item:
				...
				if cache_provider in ('Real-Debrid', 'Premiumize.me', 'Offcloud', 'TorBox'):
	...
	def resolve_internal(self, scrape_provider, item_id, url_dl, direct_debrid_link=False):
		...
		try:
			if direct_debrid_link or scrape_provider == 'folders': url = url_dl
				...
				if any(i in scrape_provider for i in ('rd_', 'oc_', 'tb_')):
