...
def get_key_id(params):
	...
	if search_type == 'people': return Images().run({'mode': 'tmdb_people_search_image_results', 'key_id': key_id, 'page_no': 1})
	if search_type == 'easynews_image': return search_easynews_image(key_id)
	url_params.update({'key_id': key_id, 'name': 'Search Results for %s' % key_id})
...
def clear_search():
	...
	('Clear Easynews Video Search History', 'easynews_video_queries'),
	('Clear Easynews Image Search History', 'easynews_image_queries'),
